<?php
	...
	public function registerView()
	{
		...
		return View::make('home.register')->with('sexes',$sexes)->with('countries',$countries);
				 ...
				 Mail::send('emails.welcome', $email_data, function($message) use ($email_data)
				 	{
				 	  ...
				 	  $message->from('support@ntradex.com', 'Ntradex Support');
				 	  $message->to($email_data['email'], $email_data['last_name'].' '.$email_data['first_name'])->subject('Welcome to Ntradex');
		...
		if($validator->passes())
			{
				$user = HomeModel::check($inputs);
				 if (!is_null($user))
					...
					{
						Session::flash('login_message','Username or Password is wrong');
			        	return Redirect::to('/login');
			        }
			 }
		 else
